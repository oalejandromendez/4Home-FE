...
export class PromocodeComponent implements OnInit, OnDestroy {
  ...
  loadForm() {
    this.form = new FormGroup({
      amount: new FormControl('', [Validators.required, Validators.max(200)]),
      reward: new FormControl('', [Validators.required, Validators.max(100)]),
      expires: new FormControl('', [Validators.required, Validators.max(365)]),
      quantity: new FormControl('', [Validators.required, Validators.max(500)]),
      disposable: new FormControl(false, [Validators.required]),
    });
  }
  ...
  loadData() {
    const that = this;
    this.promocodeService.get().subscribe(resp => {
      for( let index in resp) {
        that.promocodes.push(resp[index]);
      }
      this.dtTrigger.next();
  ...
  loadTable() {
    ...
    this.dtOptions = {
      ...
      columnDefs: [
        ...
        { targets: 6,  className: 'wid-15 text-center' }
      ],
      order: [],
  ...
  onSubmit() {
    ...
    this.promocode = this.form.value;

    this.promocodeService.post( this.promocode ).subscribe( (data: any) => {
      Swal.close();
      const toastOptions: ToastOptions = {
        title: '¡Proceso Exitoso!',
        msg: 'Los códigos promocionales se han registrado exitosamente',
        showClose: false,
        timeout: 3000,
        theme: 'bootstrap',
      };
  ...
  delete(id: any, index: any) {
    if (id) {
      Swal.fire({
        title: '¿ Está seguro ?',
        text:  'Usted no podrá recuperar los datos eliminados',
        icon: 'question',
        ...
        preConfirm: () => {
          return new Promise<void>((resolve) => {
            this.promocodeService.patch(id).subscribe( data => {
              this.promocodes.splice(index, 1);
              this.dtOptions = {};
              this.loadTable();
              this.rerender();
              this.cancel();
              Swal.fire('Proceso Exitoso!', 'Se ha eliminado el código exitosamente', 'success' );
            }, (err: any) => {
  ...
  getPermissions() {
    ...
    this.userService.permissions().subscribe( resp => {
      const create = resp.filter( (permission: any) => permission.name === 'CREAR_CODIGOS_PROMOCIONALES');
      if(create.length >= 1) {
        that.canCreate = true;
      }
      const see = resp.filter( (permission: any) => permission.name === 'VER_CODIGOS_PROMOCIONALES');
      if(see.length >= 1) {
        that.canSee = true;
      }
      const edit = resp.filter( (permission: any) => permission.name === 'MODIFICAR_CODIGOS_PROMOCIONALES');
      if(edit.length >= 1) {
        that.canEdit = true;
      }
      const deleteP = resp.filter( (permission: any) => permission.name === 'ELIMINAR_CODIGOS_PROMOCIONALES');
      if(deleteP.length >= 1) {
        that.canDelete = true;
      }
      if(!that.canCreate) {
