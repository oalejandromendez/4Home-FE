...
export class HistoryReportComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  getCustomers() {
    ...
    this.customerService.get().subscribe( (resp: any) => {
      resp.map( (customer: any) => {
        this.customers.push({ value: String(customer.id), label: customer.name + " " + customer.lastname } );
  ...
  onSubmit() {
    ...
    const user = this.form.get('customer').value ?? null;
    ...
    this.reportService.history({init, end, user}).subscribe( (data: any) => {
      if(data.length > 0) {
        this.reserves = data;
      } else {
